...
def validate_gmrit_email(email):
    """Validate GMRIT email format: XX34XA12XX@gmrit.edu.in (where X = digit, last second can be alphanumeric)"""
    pattern = r'^\d{2}34\d[aA]12\w\d@gmrit\.edu\.in$'
    return re.match(pattern, email) is not None

def validate_student_number(student_number):
    """Validate student number format: XX34XA12XX (where X = digit, last second can be alphanumeric)"""
    pattern = r'^\d{2}34\d[aA]12\w\d$'
    ...
    if request.method == 'POST':
        try:
            ...
            if not validate_gmrit_email(email):
                flash('Invalid email format! Must be XX34XA12XX@gmrit.edu.in (where X is a digit, last second can be alphanumeric)', 'error')
                return render_template('student_register.html')
            
            if not validate_student_number(student_number):
                flash('Invalid student number format! Must be XX34XA12XX (where X is a digit, last second can be alphanumeric)', 'error')
                return render_template('student_register.html')
